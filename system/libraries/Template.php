...
class CI_Template
{
    ...
    private $_valid_driver_class = array(
        'widgets'
    );
	...
	public function build($view, $data = array(), $return = FALSE)
	{
        ...
        $template['widgets'] = (is_object($this->widgets) ? $this->widgets->generate() : '');

        $template['date'] = array(
                'date' => date('d'),
                'month' => date('M'),
                'year' => date('Y'),
                'fulldate' => date('l, d F Y H:i:s')
        );
	    ...
	    $this->_layout = (!empty($this->_layout) ? $this->_layout : 'main');
		// Added to $this->_data['template'] by refference
		$template['content'] = $this->_body;
	...
	{
	    $page_title = array($this->_site_title);
		// If we have some segments passed
		if ($title_segments =& func_get_args())
		{
			$title = implode($title_segments);
		}
        $page_title[]=$title;
        $this->_title = implode($this->_title_separator, $page_title);
		return $this;
	}
	...
	private function _guess_title()
	{
		...
		$title_parts = array($this->_site_title);
		...
		if ( ! in_array($this->_controller, $title_parts) AND $this->_module != $this->_controller)
		{
            $this->_ci->db->where('alias',$this->_controller);
            $query = $this->_ci->db->get('system_controllers');
            $con = ($query->num_rows() > 0) ? unserialize($query->first_row()->title) : $this->_controller;
            $this_part = ($query->num_rows() > 0) ? $con[$this->_ci->lang->lang()] : $con;
            $title_parts[] = $this_part;
		}
		...
		{
            $this->_ci->db->where('alias',$this->_module);
            $query = $this->_ci->db->get('system_modules');
            $mod = unserialize($query->first_row()->title);
			$title_parts[] = $mod[$this->_ci->lang->lang()];
		}
		
		// Glue the title pieces together using the title separator setting
		$title = implode($this->_title_separator, $title_parts);
