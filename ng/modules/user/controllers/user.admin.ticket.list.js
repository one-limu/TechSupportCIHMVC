angular.module("user").controller('userAdminTicketList', function(userSession, BACKENDASSET, $state, $rootScope, $location, $window, userService, $filter, Upload, ticketService, $timeout, $mdDialog, my_utility, my_session_thing) {
    var vm = this;
    vm.list_ticket = [];
    ...
    vm.update_data = {};
    vm.detail_data = {};
    vm.is_my = $state.is('admin.myTicket')
    vm.filter = {
        filter: { like: {}, equal: { active: 1, crm_id:(vm.is_my ? vm.user_id : '') }, range: {} },
    ...
    vm.has_del = userService.has_privileges('ticket_delete')
    vm.has_ticket_assignee = userService.has_privileges('ticket_assignee')
    vm.has_ticket_assigner = userService.has_privileges('ticket_assigner')
    ...
    vm.get_selected_ticket = function(status_baru = '', cols= '') {
        ...
        angular.forEach(vm.list_ticket.list, function(value, key) {
            if (value.checked) {
                console.log(value)
                var a = {}
                a['ticket_code'] = value.ticket_code
